...
function AuditorView() {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <form onSubmit={handleSubmit} style={{ background: 'white', padding: '40px', borderRadius: '12px', boxShadow: '0 8px 24px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ marginBottom: '30px' }}>
             ...
             <div style={{ border: '2px dashed #a0aec0', padding: '30px', textAlign: 'center', background: '#f7fafc', borderRadius: '12px' }}>
                ...
                <input type="file" accept="image/*" onChange={handleFileChange} required style={{ marginBottom: '15px' }} />
        ...
        {result && (
            <div style={{ marginTop: '40px', padding: '30px', borderRadius: '12px', background: 'white', borderTop: `8px solid ${result.score >= 90 ? '#e53e3e' : result.score >= 40 ? '#dd6b20' : '#38a169'}`, boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e2e8f0', paddingBottom: '20px', marginBottom: '20px' }}>
                    ...
                    <span style={{ fontSize: '24px', fontWeight: 'bold', padding: '8px 16px', borderRadius: '20px', backgroundColor: result.score >= 90 ? '#fff5f5' : result.score >= 40 ? '#fffff0' : '#f0fff4', color: result.score >= 90 ? '#e53e3e' : result.score >= 40 ? '#dd6b20' : '#38a169' }}>
                        {result.status}
                    </span>
                </div>
                {result.flags.length > 0 ? (
                    <div style={{ backgroundColor: '#fff5f5', padding: '20px', borderRadius: '8px', borderLeft: '4px solid #e53e3e' }}>
                        ...
                        <ul style={{ color: '#c53030', fontWeight: 'bold', fontSize: '16px', margin: 0, paddingLeft: '20px' }}>
                            {result.flags.map((flag, index) => <li key={index} style={{ marginBottom: '8px' }}>{flag}</li>)}
                        </ul>
                    </div>
                ) : (
                    <div style={{ backgroundColor: '#f0fff4', padding: '20px', borderRadius: '8px', borderLeft: '4px solid #38a169' }}>
                        <p style={{ color: '#276749', fontWeight: 'bold', fontSize: '18px', margin: 0 }}>✅ All AI checks passed.</p>
                    </div>
                )}
            </div>
        )}
